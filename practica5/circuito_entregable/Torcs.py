...
class TorcsProcessor(object):
    def __init__(self):
        self.args = {'emitter':None,'publish':False,'modus':'simple','driver':None, 'train':None}
        ...
        self.driver = None
        self.publish = False
        self.archivoTrainning = "trainning_set.txt"
        
    def __del__(self):
        self.archivoTrainning.close()
    ...
    def start(self):
        # Obtener el tipo de conductor humano: keyboard o gamepad

        if ( self.args.get('train') ):
            print("\n\nArchivo entrenamiento: %s"%(self.args.get('train')))
            self.archivoTrainning = open( self.args.get('train'), "w" )
            

        self.driver = getDriver(self.args.get('driver'), self.args.get('modus'))
    ...
    def initialize(self,port):
        return 'SCR(init %s)'%(' '.join(self.driver.angles))

    def writeSensorsActions(self, sensors, actions):
        listaSensores = ['angle', 'trackPos', 'speedX', 'speedY', 'gear', 'rpm']
        
        cadena = ""
        for clave in listaSensores:
            cadena = cadena + sensors[clave][0] + " "
           
        cadena = cadena +  "\t"
        trackSensors = sensors['track']
        cadena = cadena + " ".join(trackSensors)
       
        cadena = cadena +  "\t"       
        listaAcciones = ['accel', 'brake', 'gear', 'steer']
        
        for clave in listaAcciones:
            #print("accion=%s"%(actions[clave]))
            cadena = cadena + str(actions[clave]) + " "
       	
        #print("acciones: %s"%(actions))
        self.archivoTrainning.write(cadena + "\n")
    ...
    def process(self, message):
        ...
        ###### Aquí es donde el conductor designado recibe la información del entorno y 
        ###### decide los valores de los actuadores del auto.
        actions = self.modus(sensors)
        ######
        
        if self.args.get('train'):
            self.writeSensorsActions(sensors, actions)
...
class TorcsClient(object):
    ...
    def connect(self):
        ...
        logging.info('Aquí llegó 2 con %s.'%message)
        ...
        while not self.connected:
            try:
                ...
                logging.info("Llegó el mensaje '%s'."%message)
                santo = b'***identified***\x00'

                if message == santo:
                    logging.info('connection established to %s:%d'%a)
                    self.connected = True
                    break
        ...
        try:
            while self.connected:
                ...
                if not message:
                    ...
                    #logging.info(self.processor.process(message.decode()).encode())
